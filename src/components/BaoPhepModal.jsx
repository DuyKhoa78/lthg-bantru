...
// Tìm thứ 2 đầu tuần của ngày được chọn
...
export default function BaoPhepModal({
    ...
    // Absences list state (Theo tuần)
    const [weekMonday, setWeekMonday] = useState(() => getMonday(defaultDate || new Date().toISOString().split('T')[0]));
    ...
    // Load existing excused absence list by week
    const fetchWeekPhepList = useCallback((monIso) => {
        setLoadingList(true);
        const friIso = getFriday(monIso);
        api.get(`/api/diemdanh/danh-sach-phep/?tu=${monIso}&den=${friIso}`)
            .then(res => {
                if (res.data?.ok) {
    ...
    useEffect(() => {
        if (open && activeTab === 'list') {
            fetchWeekPhepList(weekMonday);
        }
    }, [open, activeTab, weekMonday, fetchWeekPhepList]);

    // Danh sách 5 ngày trong tuần (T2 - T6) kèm thống kê số lượng
    const weekDays = useMemo(() => {
        const DOW_NAMES = ['Thứ 2', 'Thứ 3', 'Thứ 4', 'Thứ 5', 'Thứ 6'];
        const mon = new Date(weekMonday + 'T00:00:00');
        return [0, 1, 2, 3, 4].map(i => {
            const d = new Date(mon);
            d.setDate(mon.getDate() + i);
            const iso = toISO(d);
            const count = phepList.filter(item => item.ngay === iso).length;
            return {
                iso,
                dowLabel: DOW_NAMES[i],
                shortDow: `T${i + 2}`,
                dayMonth: `${p2(d.getDate())}/${p2(d.getMonth() + 1)}`,
                count,
                d
            };
        });
    ...
    const weekFri = useMemo(() => getFriday(weekMonday), [weekMonday]);

    const weekLabel = useMemo(() => {
        const m = new Date(weekMonday + 'T00:00:00');
        const f = new Date(weekFri + 'T00:00:00');
        return `${p2(m.getDate())}/${p2(m.getMonth() + 1)} – ${p2(f.getDate())}/${p2(f.getMonth() + 1)}/${f.getFullYear()}`;
    }, [weekMonday, weekFri]);

    // Danh sách hiển thị sau khi lọc theo ngày
    const displayList = useMemo(() => {
        if (dayFilter === 'all') return phepList;
        return phepList.filter(item => item.ngay === dayFilter);
    }, [phepList, dayFilter]);
    ...
    const handlePrevWeek = () => {
        ...
        const prevMon = toISO(m);
        setWeekMonday(prevMon);
        setDayFilter('all');
        fetchWeekPhepList(prevMon);
    };

    const handleNextWeek = () => {
        const m = new Date(weekMonday + 'T00:00:00');
        m.setDate(m.getDate() + 7);
        const nextMon = toISO(m);
        setWeekMonday(nextMon);
        setDayFilter('all');
        fetchWeekPhepList(nextMon);
    };

    const handleCurrentWeek = () => {
        const todayIso = new Date().toISOString().split('T')[0];
        const thisMon = getMonday(todayIso);
        setWeekMonday(thisMon);
        setDayFilter('all');
        fetchWeekPhepList(thisMon);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.data?.ok) {
                ...
                setSelectedHsIds(new Set());
                onSuccess?.();
                // Chuyển sang xem thống kê danh sách tuần chứa ngày vừa báo
                const targetMon = getMonday(startDate);
                setWeekMonday(targetMon);
                setDayFilter('all');
                setActiveTab('list');
                fetchWeekPhepList(targetMon);
            } else {
    ...
    const handleCancelPhep = async (item) => {
        if (!window.confirm(`Bạn có chắc muốn hủy vắng phép cho học sinh ${item.hoc_sinh?.ho_ten || item.ma_hs_id} ngày ${fmtDate(item.ngay)}?`)) {
            return;
        }
    ...
    return (
        <>
            ...
            <div className="modal-overlay open" style={{ zIndex: 99999 }} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
                <div className="modal-box" style={{ maxWidth: 880, width: '95vw', maxHeight: '92vh', display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden', borderRadius: 16 }}>
                    ...
                    <div style={{
                        ...
                        >
                            <i className="fas fa-calendar-week"></i> Danh sách &amp; Thống kê tuần
                            {phepList.length > 0 && (
                                <span style={{
                                    background: activeTab === 'list' ? '#009CFF' : '#cbd5e1',
                                    color: '#fff', borderRadius: 10, padding: '1px 7px', fontSize: '0.75rem', fontWeight: 800
                                }}>
                                    {phepList.length}
                                </span>
                            )}
                        ...
                        {activeTab === 'record' ? (
                            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                                ...
                                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12, padding: 16 }}>
                                    ...
                                    <div style={{
                                        maxHeight: 190, overflowY: 'auto', border: '1px solid #e2e8f0',
                                                    ...
                                                    {filteredStudents.map(s => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                ...
                                                                <td style={{ textAlign: 'center', color: '#0369a1', padding: '6px 8px' }}>{s.phong_an || '—'}</td>
                                                                <td style={{ textAlign: 'center', color: '#7c3aed', padding: '6px 8px' }}>{s.phong_ngu || '—'}</td>
                                                            </tr>
                                                        );
                                                    })}
                                                </tbody>
                            ...
                            /* TAB 2: DANH SÁCH & THỐNG KÊ VẮNG PHÉP THEO TUẦN */
                            <div>
                                ...
                                <div style={{
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                        ...
                                        <div style={{
                                            ...
                                            <i className="far fa-calendar-alt" style={{ color: '#009CFF' }}></i>
                                        ...
                                        <button
                                            ...
                                            style={{ padding: '5px 10px', fontSize: '0.82rem', color: '#009CFF', fontWeight: 600 }}
                                        >
                                            Tuần này
                                        </button>
                                    </div>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                        <button
                                            type="button"
                                            className="btn btn-outline btn-sm"
                                            onClick={() => fetchWeekPhepList(weekMonday)}
                                        ...
                                        <span style={{ fontSize: '0.88rem', color: '#64748b' }}>
                                            Tổng tuần: <strong style={{ color: '#d97706', fontSize: '1rem' }}>{phepList.length}</strong> lượt vắng
                                        </span>
                                    </div>
                                </div>

                                {/* BỘ THỐNG KÊ TUẦN (Bấm từng ngày để lọc danh sách) */}
                                <div style={{
                                    display: 'grid',
                                    ...
                                    <div
                                        ...
                                        style={{
                                            ...
                                            borderRadius: 8,
                                            textAlign: 'center',
                                            border: dayFilter === 'all' ? '2px solid #009CFF' : '1px solid #e2e8f0',
                                            background: dayFilter === 'all' ? '#e0f2fe' : '#fff',
                                            transition: 'all 0.15s ease'
                                        }}
                                    >
                                        <div style={{ fontSize: '0.78rem', color: dayFilter === 'all' ? '#0369a1' : '#64748b', fontWeight: 700 }}>
                                            Cả tuần
                                        </div>
                                        <div style={{
                                            fontSize: '1.2rem',
                                            fontWeight: 800,
                                            color: dayFilter === 'all' ? '#0284c7' : '#1e293b',
                                            marginTop: 2
                                        }}>
                                            {phepList.length}
                                        </div>
                                    ...
                                    {/* 5 ngày T2 - T6 */}
                                    {weekDays.map(wd => {
                                        const isSelected = dayFilter === wd.iso;
                                        const hasCount = wd.count > 0;
                                        return (
                                            <div
                                                key={wd.iso}
                                                onClick={() => setDayFilter(wd.iso)}
                                                style={{
                                                    cursor: 'pointer',
                                                    padding: '8px 8px',
                                                    borderRadius: 8,
                                            ...
                                            >
                                                <div style={{
                                                    fontSize: '0.78rem',
                                                    color: isSelected ? '#0369a1' : hasCount ? '#b45309' : '#64748b',
                                                    fontWeight: 700
                                                }}>
                                                    {wd.dowLabel} <span style={{ fontWeight: 400, fontSize: '0.72rem' }}>({wd.dayMonth})</span>
                                                </div>
                                                <div style={{
                                                    fontSize: '1.2rem',
                                                    fontWeight: 800,
                                ...
                                {loadingList ? (
                                    <div style={{ textAlign: 'center', padding: 40, color: '#64748b' }}>
                                        ...
                                        <p>Đang tải danh sách vắng phép tuần...</p>
                                    </div>
                                ) : displayList.length === 0 ? (
                                    <div style={{ textAlign: 'center', padding: 40, color: '#94a3b8', background: '#f8fafc', borderRadius: 10, border: '1px dashed #cbd5e1' }}>
                                        <i className="fas fa-user-check" style={{ fontSize: '2.5rem', color: '#cbd5e1', marginBottom: 10 }}></i>
                                        <h4 style={{ color: '#475569', margin: '0 0 6px', fontSize: '0.95rem' }}>
                                            {dayFilter === 'all'
                                                ? 'Chưa có học sinh nào báo vắng phép trong tuần này'
                                                : `Không có học sinh nào xin nghỉ phép ngày ${fmtDate(dayFilter)}`}
                                        </h4>
                                        <p style={{ margin: 0, fontSize: '0.82rem' }}>
                                            {dayFilter === 'all'
                                                ? `Tuần ${weekLabel} chưa ghi nhận học sinh nào vắng phép.`
                                                : 'Tất cả học sinh đều có mặt bình thường trong ngày này.'}
                                        </p>
                                    </div>
                                ) : (
                                        ...
                                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                                            <thead>
                                                <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0', color: '#475569' }}>
                                                    ...
                                                    <th style={{ width: 130, padding: '10px 8px', textAlign: 'center' }}>Ngày vắng</th>
                                                    ...
                                                    <th style={{ width: 140, padding: '10px 8px', textAlign: 'center' }}>Ca vắng</th>
                                            ...
                                            <tbody>
                                                {displayList.map((item, idx) => {
                                                    ...
                                                    const dObj = new Date(item.ngay + 'T00:00:00');
                                                    const dowIdx = dObj.getDay();
                                                    const dowStr = dowIdx === 0 ? 'CN' : `Thứ ${dowIdx + 1}`;
                                                    const ngayStr = `${fmtDate(item.ngay)} (${dowStr})`;

                                                    return (
                                                        <tr key={item.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                                            <td style={{ textAlign: 'center', padding: '10px 8px', color: '#64748b' }}>{idx + 1}</td>
                                                            <td style={{ textAlign: 'center', padding: '10px 8px', fontWeight: 700, color: '#1e293b' }}>
                                                                <span style={{
                                                                    background: '#f1f5f9', padding: '3px 8px', borderRadius: 6,
                                                                    fontSize: '0.8rem', color: '#334155', display: 'inline-flex', alignItems: 'center', gap: 4
                                                                }}>
                                                                    <i className="far fa-calendar" style={{ color: '#009CFF', fontSize: '0.75rem' }}></i>
                                                                    {ngayStr}
                                                                </span>
                                                            </td>
                                                            <td style={{ textAlign: 'center', fontWeight: 800, color: '#c00', padding: '10px 8px', fontSize: '0.92rem' }}>{item.ma_hs_id}</td>
                                                            <td style={{ fontWeight: 600, color: '#1e293b', padding: '10px 8px' }}>{hs?.ho_ten || 'Học sinh #' + item.ma_hs_id}</td>
                                                            <td style={{ textAlign: 'center', padding: '10px 8px', fontWeight: 600 }}>{hs?.lop || '—'}</td>
                                                            <td style={{ textAlign: 'center', color: '#0369a1', padding: '10px 8px', fontWeight: 600 }}>{hs?.phong_an || '—'}</td>
                                                            <td style={{ textAlign: 'center', color: '#7c3aed', padding: '10px 8px', fontWeight: 600 }}>{hs?.phong_ngu || '—'}</td>
                                                            <td style={{ textAlign: 'center', padding: '10px 8px' }}>
                                                                <span style={{
                                                                    background: '#fffbeb', border: '1px solid #fde68a', color: '#b45309',
                                                                    padding: '3px 8px', borderRadius: 6, fontSize: '0.78rem', fontWeight: 700
                                                                }}>
                                                                    {caLabel}
                                                                </span>
                                                            </td>
