...
export default function HocSinh() {
    ...
    // Tải file mẫu CSV chuẩn cho Học sinh (có sep=, để Excel tự động tách cột A, B, C...)
    ...
    // ── Xuất Excel (.xlsx) danh sách học sinh chuẩn Microsoft Excel ──
    const handleExportExcel = (targetLop = null, onlyDangHoc = false) => {
        let list = data;
        if (onlyDangHoc) {
            list = list.filter(s => Boolean(s.dang_hoc));
        }
        if (targetLop) {
            list = list.filter(s => s.lop === targetLop);
        }
        if (list.length === 0) {
            return showAlert(targetLop ? `Không có học sinh nào phù hợp tại lớp ${targetLop}` : 'Không có dữ liệu học sinh để xuất Excel', 'warning');
        }

        // Sắp xếp: Lớp -> Tên học sinh (alphabet tiếng Việt)
        list = [...list].sort((a, b) => {
        ...
        const d = new Date();
        const todayStr = `Ngày ${d.getDate()} tháng ${d.getMonth() + 1} năm ${d.getFullYear()}`;
        const tenTruong = cauhinh?.ten_truong || 'THPT LÊ THỊ HỒNG GẤM';
        const namHoc = cauhinh?.nam_hoc || '2026-2027';

        const headerRows = [
            [`TRƯỜNG ${tenTruong.toUpperCase()} - BỘ PHẬN QUẢN LÝ BÁN TRÚ`],
            [targetLop ? `DANH SÁCH HỌC SINH BÁN TRÚ LỚP ${targetLop.toUpperCase()}` : 'DANH SÁCH TOÀN BỘ HỌC SINH BÁN TRÚ'],
            [`Năm học: ${namHoc} | Xuất ngày: ${todayStr} | Tổng số: ${list.length} học sinh (${list.filter(s => s.dang_hoc).length} đang học, ${list.filter(s => !s.dang_hoc).length} đã rút)`],
            [],
            ['STT', 'Mã BT', 'Họ và tên', 'Giới tính', 'Lớp', 'Phòng ăn', 'Phòng ngủ', 'Ngày vào', 'Trạng thái', 'Ghi chú']
        ];
        ...
        const dataRows = list.map((s, idx) => {
            ...
            const tt = s.dang_hoc ? 'Đang học' : 'Đã rút';
            return [
                idx + 1,
                s.id,
                s.ho_ten,
        ...
        const sheetName = targetLop ? `Lop_${targetLop}`.substring(0, 31) : 'DS_Tat_Ca_HS';
        XLSX.utils.book_append_sheet(wb, ws, sheetName);

        // Nếu xuất toàn trường, tự động tạo thêm từng sheet riêng cho từng lớp
        if (!targetLop) {
            const distinctLops = [...new Set(list.map(s => s.lop))].filter(Boolean).sort(compareClasses);
            distinctLops.forEach(lop => {
                const lopStudents = list.filter(s => s.lop === lop);
                const lopHeaders = [
                    [`DANH SÁCH HỌC SINH BÁN TRÚ LỚP ${lop.toUpperCase()}`],
                    [`Sĩ số: ${lopStudents.length} học sinh | Năm học: ${namHoc}`],
                    [],
                    ['STT', 'Mã BT', 'Họ và tên', 'Giới tính', 'Lớp', 'Phòng ăn', 'Phòng ngủ', 'Ngày vào', 'Trạng thái', 'Ghi chú']
                ];
                const lopRows = lopStudents.map((s, idx) => [
                    idx + 1,
                    s.id,
                    s.ho_ten,
                    s.gioi_tinh === 0 ? 'Nam' : 'Nữ',
                    s.lop || '',
                    s.phong_an?.ma_phong || s.ma_phong_an_id || '',
                    s.phong_ngu?.ma_phong || s.ma_phong_ngu_id || '',
                    formatVnDate(s.ngay_vao) || '',
                    s.dang_hoc ? 'Đang học' : 'Đã rút',
                    s.ghi_chu || ''
                ]);
                const lopWs = XLSX.utils.aoa_to_sheet([...lopHeaders, ...lopRows]);
                lopWs['!cols'] = ws['!cols'];
                XLSX.utils.book_append_sheet(wb, lopWs, `Lop_${lop}`.substring(0, 31));
            });
        }

        const todayIso = new Date().toISOString().slice(0, 10);
        const fileName = targetLop
            ? `Danh_Sach_HS_Ban_Tru_Lop_${targetLop}_${todayIso}.xlsx`
            : `Danh_Sach_Tat_Ca_Hoc_Sinh_Ban_Tru_${todayIso}.xlsx`;

        XLSX.writeFile(wb, fileName);
        showAlert(targetLop ? `Đã xuất ${list.length} học sinh lớp ${targetLop} ra file Excel chuẩn (.xlsx)!` : `Đã xuất toàn bộ ${list.length} học sinh ra file Excel (.xlsx) chuẩn gồm danh sách tổng hợp và từng sheet theo lớp!`, 'success');
    };

    // ── Xuất CSV danh sách học sinh bán trú (Khắc phục triệt để lỗi dồn cột A trên Excel) ──
    const handleExportCsv = (targetLop = null, onlyDangHoc = false) => {
        let list = data;
        if (onlyDangHoc) {
            list = list.filter(s => Boolean(s.dang_hoc));
        }
        ...
        if (list.length === 0) {
            return showAlert(targetLop ? `Không có học sinh nào phù hợp tại lớp ${targetLop}` : 'Không có học sinh nào phù hợp', 'warning');
        }
        ...
        // UTF-8 BOM \uFEFF chuẩn đảm bảo tiếng Việt hiển thị chính xác không bị lỗi font
        ...
        const fileName = targetLop ? `Danh_Sach_HS_Ban_Tru_Lop_${targetLop}_${today}.csv` : `Danh_Sach_Tat_Ca_HS_Ban_Tru_${today}.csv`;
        a.download = fileName;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        showAlert(targetLop ? `Đã xuất danh sách ${list.length} học sinh lớp ${targetLop} ra file CSV!` : `Đã xuất danh sách ${list.length} học sinh ra file CSV (đã xử lý tách cột cho Excel)!`, 'success');
    };
            ...
            <div className="page-header">
                ...
                <div className="page-header-actions">
                    <button
                        ...
                        title="Xuất danh sách học sinh ra file Excel / CSV / PDF"
                    >
                        <i className="fas fa-file-export" style={{ color: '#16a34a' }}></i> Xuất danh sách HS
                    </button>
                    <Link
                        to="/in-the-ban-tru"
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6, flexWrap: 'wrap', gap: 8 }}>
                                ...
                                <div style={{ display: 'flex', gap: 8 }}>
                                    <button
                                        ...
                                        style={{ fontSize: '.78rem', padding: '4px 10px', borderRadius: 6, fontWeight: 700, background: '#15803d', borderColor: '#15803d', color: '#fff' }}
                                        title="Xuất file bảng tính Excel (.xlsx) danh sách học sinh hiện tại chuẩn từng cột"
                                    >
                                        <i className="fas fa-file-excel" style={{ marginRight: 4 }}></i> Xuất Excel (.xlsx) hiện tại
                                    </button>
                                    <button
                                        type="button"
                    ...
                    <div className="modal-box modal-lg" style={{ maxWidth: 850 }}>
                        <div className="modal-header">
                            <div className="modal-title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <i className="fas fa-file-export" style={{ color: '#2563eb' }}></i>
                                <span>Xuất danh sách học sinh (Excel / PDF)</span>
                            </div>
                        ...
                        <div className="modal-footer" style={{ justifyContent: 'flex-end', gap: 10, flexWrap: 'wrap' }}>
                            <button
                                className="btn btn-success"
                                onClick={() => handleExportExcel(exportSelectedLop, exportOnlyDangHoc)}
                                disabled={previewStudents.length === 0}
                                style={{ background: '#15803d', borderColor: '#15803d', fontWeight: 700 }}
                                title="Tải file bảng tính Excel (.xlsx) chuẩn Microsoft Excel từng cột rõ ràng"
                            >
                                <i className="fas fa-file-excel"></i> Xuất Excel (.xlsx)
                            </button>
                            <button
                                className="btn btn-outline-success"
                                onClick={() => handleExportCsv(exportSelectedLop, exportOnlyDangHoc)}
                                disabled={previewStudents.length === 0}
                                style={{ fontWeight: 600, borderColor: '#86efac', color: '#16a34a' }}
                                title="Tải file CSV đã tách cột chuẩn cho Excel"
                            >
                            ...
                            >
                                ...
                                {exportingPdf ? ` ${downloadProgress || 'Đang xuất PDF...'}` : ' Tải PDF'}
                            </button>
                        </div>
                    </div>
                </div>
