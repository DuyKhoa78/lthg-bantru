...
export default function BaoCaoTrucGV() {
    ...
    // Helper: Đếm số HS vắng từ chuỗi danh_sach_vang
    const countVang = (r) => {
        if (!r) return 0;
        const ds = r.danh_sach_vang;
        if (!ds || !ds.trim()) return Number(r.so_hs_vang) || 0;
        const lines = ds.split(/\r?\n|;/).filter(Boolean);
        if (lines.length > 1) return lines.length;
        const classMatches = ds.match(/\d{2}[A-Za-z]+\d*/g);
    ...
    const filteredRecords = useMemo(() => {
        ...
        } else if (activeSection === 'hs_vang') {
            list = list.filter(r => r.danh_sach_vang && r.danh_sach_vang.trim() && r.ca_truc !== 2);
        }
    ...
    const exportLeaderReportPdf = () => {
        ...
        const totalHsVang = (data.records || []).reduce((sum, r) => sum + countVang(r), 0);
      ...
      `;
        ...
        <tr>
          ...
          <td style="font-size:8.5pt;">
            ${soVang > 0 ? `
              <div style="color:#1e293b; line-height:1.45; font-style:italic;">
                ${(() => {
                    const ds = r.danh_sach_vang || '';
                    const lines = ds.split(/\r?\n|;/).filter(Boolean);
                    if (lines.length > 1) return lines.map(l => l.trim()).join('<br>');
                    // Tách theo mã lớp: "Lưu Diệc Phi 10A1 Lâm Bình Chi 10A2" → từng dòng
                    return ds.replace(/(\d{2}[A-Za-z]+\d*)/g, '$1<br>').replace(/<br>\s*$/, '').trim();
                })()}
              </div>
            ` : `
          ...
          <td style="font-size:9pt; color:#334155;">
            <div>${r.tinh_hinh && r.tinh_hinh.trim() ? r.tinh_hinh : 'Tốt, trật tự'}</div>
            ${hasViPham ? `<div style="color:#b91c1c; font-weight:600; margin-top:3px; font-size:8.5pt;">⚠ Vi phạm: ${r.hs_vi_pham}</div>` : ''}
          </td>
...
<body>
  ...
  <div style="font-size:10pt; margin-bottom:10px; line-height:1.45;">
    ...
    <div style="margin-left:15px; margin-top:3px;">
      ...
      - Tình hình học sinh vắng trong ca trực: <strong>${totalHsVang} lượt</strong> học sinh vắng.
    </div>
  </div>

  <div class="sec-part-title">
    II. BẢNG TỔNG HỢP CHI TIẾT CÁC CA TRỰC / PHÒNG BÁN TRÚ:
  </div>
  ...
  <table class="data-table">
    <thead>
      <tr>
        ...
        <th style="width:65px;">Sĩ số</th>
        <th style="width:145px;">Học sinh vắng</th>
        <th>Tình hình nề nếp</th>
        <th style="width:110px;">Ghi chú / Kiến nghị</th>
      </tr>
    </thead>
    <tbody>
      ${renderRoomRows(anRecords, 'Ăn')}
      <tr style="background:#f8fafc; font-weight:bold; border-top:1.5px solid #333;">
        <td colspan="4" style="text-align:center; font-weight:800; font-size:9pt;">TỔNG CỘNG (${anRecords.length} phòng)</td>
        <td style="text-align:center; font-weight:800; font-size:9.5pt; color:#0f172a;">${anRecords.reduce((s, r) => s + (parseInt(r.si_so) || 0), 0) || '—'}</td>
        <td style="font-weight:700; font-size:8.5pt; color:#b91c1c;">${(() => { const v = anRecords.reduce((s, r) => s + countVang(r), 0); return v > 0 ? `Vắng: ${v} HS` : '<span style="color:#16a34a;">✓ Đủ sĩ số</span>'; })()}</td>
        <td style="font-size:8.5pt; color:#334155;">—</td>
        <td style="font-size:8.5pt; color:#475569;">—</td>
      </tr>
    </tbody>
  </table>
  ...
  <table class="data-table">
    ...
    <tbody>
      ...
      <tr style="background:#f8fafc; font-weight:bold; border-top:1.5px solid #333;">
        <td colspan="4" style="text-align:center; font-weight:800; font-size:9pt;">TỔNG CỘNG (${nguRecords.length} phòng)</td>
        <td style="text-align:center; font-weight:800; font-size:9.5pt; color:#0f172a;">${nguRecords.reduce((s, r) => s + (parseInt(r.si_so) || 0), 0) || '—'}</td>
        <td style="font-weight:700; font-size:8.5pt; color:#b91c1c;">${(() => { const v = nguRecords.reduce((s, r) => s + countVang(r), 0); return v > 0 ? `Vắng: ${v} HS` : '<span style="color:#16a34a;">✓ Đủ sĩ số</span>'; })()}</td>
        <td style="font-size:8.5pt; color:#334155;">—</td>
        <td style="font-size:8.5pt; color:#475569;">—</td>
      </tr>
    </tbody>
  ...
  <script>window.onload = function() { setTimeout(window.print, 400); };</script>
    ...
    const exportDailyReportPdf = () => {
        ...
        <tr>
          ...
          <td style="text-align:center; font-weight:bold; color:#0f172a;">${r.si_so || '—'}</td>
          <td style="font-size:9pt;">
            ${soVang > 0 ? `
              <div style="color:#1e293b; font-style:italic; line-height:1.45;">
                ${(() => {
                    const ds = r.danh_sach_vang || '';
                    const lines = ds.split(/\r?\n|;/).filter(Boolean);
                    if (lines.length > 1) return lines.map(l => l.trim()).join('<br>');
        ...
        <tr>
          ...
          <td style="font-size:8.5pt; color:#334155;">${r.tinh_hinh || 'Bình thường'}</td>
        </tr>`).join('');
        };
...
<body>
  ...
  <table class="data-table">
    <thead>
      <tr>
        <th style="width:35px;">STT</th>
        <th style="width:85px;">Ngày trực</th>
        <th style="width:75px;">Phòng ăn</th>
        <th style="width:65px;">Sĩ số</th>
        <th style="width:145px;">Học sinh vắng</th>
        <th style="width:130px;">Giáo viên trực</th>
        <th>Tình hình nề nếp chung</th>
        <th style="width:120px;">Ghi chú / Kiến nghị</th>
      </tr>
    </thead>
    <tbody>
      ${renderRoomRows(anRecords, 'Ăn')}
  ...
  <script>window.onload = function() { setTimeout(window.print, 400); };</script>
    ...
    // Code mẫu Apps Script chuẩn mới nhất có Danh sách HS vắng
    const sampleScript = `/**
 * =========================================================================================
 * GOOGLE APPS SCRIPT - BÁO CÁO CA TRỰC BÁN TRÚ THPT LÊ THI HỒNG GẤM (CHUẨN CÓ HS VẮNG)
 * Hỗ trợ Form 3 phần (Trực ăn, Trực ngủ & Giám sát) kèm cột "Danh sách HS vắng"
 * Dán code này vào Google Trang tính hoặc Google Form -> Tiện ích mở rộng -> Apps Script
 * =========================================================================================
 */
...
/**
 * -----------------------------------------------------------------------------------------
 * 🚀 CÁCH 1: DÀNH CHO GOOGLE SHEETS (Khuyên dùng khi mở Apps Script từ Google Trang tính)
...
function onSheetSubmit(e) {
  try {
    const row = e.values || [];
    if (!row || row.length === 0) {
      Logger.log("Không có dữ liệu dòng!");
      return;
    }

    // Thứ tự các cột chuẩn mới nhất (có cột Danh sách HS vắng):
    // [0] Dấu thời gian | [1] Ca trực
    // [Giám sát] [2] Họ tên | [3] Vị trí/Phòng | [4] Tình hình nề nếp | [5] Vệ sinh an toàn thực phẩm
    ...
    // Nhận diện cấu trúc 18 cột mới (có danh sách HS vắng)
    const isNew18Col = row.length >= 18 || Boolean(row[13]) || (Boolean(row[6]) && Boolean(row[10]) && Boolean(row[11]));
    const isOld17Col = !isNew18Col && (row.length >= 17 || Boolean(row[12]) || Boolean(row[11]));

    // 1. Nhánh Giám sát
    if (ca_truc_raw.includes("giám sát") || ca_truc_raw.includes("gám sát") || ca_truc_raw.includes("giamsat") || (Boolean(row[2]) && !row[6] && !row[11] && !row[12])) {
      ca_truc = "Giám sát";
      ho_ten_gv = row[2] || "";
    ...
    // 2. Nhánh Ca ngủ
    else if (ca_truc_raw.includes("ngủ") || ca_truc_raw.includes("nghi") || ca_truc_raw.includes("nghỉ") || Boolean(row[12])) {
      ca_truc = "Trực ngủ";
      if (isNew18Col) {
        ho_ten_gv = row[12] || "";
        ma_phong = row[13] || "";
        
        // Tự động nhận diện nếu cột Sĩ số và Tình hình bị đổi chỗ
    ...
    // Tự động đếm số học sinh vắng từ danh sách
    if (danh_sach_vang) {
...
/**
 * -----------------------------------------------------------------------------------------
 * 🚀 CÁCH 2: DÀNH CHO GOOGLE FORM (Nếu bạn mở Apps Script trực tiếp từ Google Form)
 * Tạo Trigger (Trình kích hoạt):
...
function onFormSubmit(e) {
  if (!e || !e.response) {
    Logger.log("⚠️ Hàm này chạy tự động khi có người gửi Form.");
    return;
  }
...
/**
 * -----------------------------------------------------------------------------------------
 * ⚡ HÀM TEST (CHẠY THỬ): Bấm nút 'Chạy' (Run) hàm này để test gửi dòng cuối cùng trong Sheet
 * -----------------------------------------------------------------------------------------
 */
function testSendLatestSheetRow() {
  try {
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet();
    const lastRow = sheet.getLastRow();
    if (lastRow <= 1) {
      Logger.log("Chưa có dòng dữ liệu nào trong Sheet để test!");
      return;
    }
    const rowValues = sheet.getRange(lastRow, 1, 1, sheet.getLastColumn()).getValues()[0];
    Logger.log("Đang test dòng " + lastRow + ": " + JSON.stringify(rowValues));
    onSheetSubmit({ values: rowValues });
  } catch (err) {
    Logger.log("Lỗi test Sheet: " + err.toString());
  }
}`;
    ...
    return (
        <div className="bctruc-container">
            ...
            {/* ── Thanh thống kê nhanh gọn (1 dòng, bấm để lọc) ── */}
            <div className="bctruc-card" style={{ marginBottom: 18, padding: '10px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                    {/* Tổng báo cáo */}
                    <button
                        className={`bctruc-mini-stat ${activeSection === 'all' ? 'active' : ''}`}
                        onClick={() => setActiveSection('all')}
                        style={{ '--ms-color': '#3b82f6', '--ms-bg': '#eff6ff', '--ms-border': '#bfdbfe' }}
                        title="Xem tất cả"
                    >
                        <i className="fas fa-clipboard-list"></i>
                        <span className="bctruc-mini-stat-val">{data.stats?.total || 0}</span>
                        <span className="bctruc-mini-stat-lbl">báo cáo</span>
                    </button>

                    <span className="bctruc-mini-sep">|</span>

                    {/* Ca Ăn */}
                    <button
                        className={`bctruc-mini-stat ${activeSection === 'ca_an' ? 'active' : ''}`}
                        onClick={() => setActiveSection('ca_an')}
                        style={{ '--ms-color': '#d97706', '--ms-bg': '#fffbeb', '--ms-border': '#fde68a' }}
                        title="Lọc Ca Ăn"
                    >
                        <i className="fas fa-utensils"></i>
                        <span className="bctruc-mini-stat-val">{data.stats?.caAnCount || 0}</span>
                        <span className="bctruc-mini-stat-lbl">
                            Ca Ăn
                            {viewMode === 'day' && data.stats?.totalPhongAn ? <span className="bctruc-mini-stat-sub">/ {data.stats.totalPhongAn}</span> : null}
                        </span>
                    </button>

                    {/* Ca Ngủ */}
                    <button
                        className={`bctruc-mini-stat ${activeSection === 'ca_ngu' ? 'active' : ''}`}
                        onClick={() => setActiveSection('ca_ngu')}
                        style={{ '--ms-color': '#7c3aed', '--ms-bg': '#faf5ff', '--ms-border': '#ddd6fe' }}
                        title="Lọc Ca Ngủ"
                    >
                        <i className="fas fa-bed"></i>
                        <span className="bctruc-mini-stat-val">{data.stats?.caNguCount || 0}</span>
                        <span className="bctruc-mini-stat-lbl">
                            Ca Ngủ
                            {viewMode === 'day' && data.stats?.totalPhongNgu ? <span className="bctruc-mini-stat-sub">/ {data.stats.totalPhongNgu}</span> : null}
                        </span>
                    </button>

                    {/* Giám sát */}
                    <button
                        className={`bctruc-mini-stat ${activeSection === 'giam_sat' ? 'active' : ''}`}
                        onClick={() => setActiveSection('giam_sat')}
                        style={{ '--ms-color': '#0d9488', '--ms-bg': '#f0fdfa', '--ms-border': '#99f6e4' }}
                        title="Lọc Giám sát"
                    >
                        <i className="fas fa-user-shield"></i>
                        <span className="bctruc-mini-stat-val">{data.stats?.giamSatCount || 0}</span>
                        <span className="bctruc-mini-stat-lbl">Giám sát</span>
                    </button>

                    {/* Bảng công GV */}
                    <button
                        className={`bctruc-mini-stat ${activeSection === 'giao_vien' ? 'active' : ''}`}
                        onClick={() => setActiveSection('giao_vien')}
                        style={{ '--ms-color': '#16a34a', '--ms-bg': '#f0fdf4', '--ms-border': '#bbf7d0' }}
                        title="Xem Bảng công GV"
                    >
                        <i className="fas fa-chalkboard-teacher"></i>
                        <span className="bctruc-mini-stat-val">{data.stats?.gvSummary?.length || 0}</span>
                        <span className="bctruc-mini-stat-lbl">GV trực</span>
                    </button>

                    {/* HS Vắng */}
                    <button
                        className={`bctruc-mini-stat ${activeSection === 'hs_vang' ? 'active' : ''} ${totalAbsentCount > 0 ? 'warn' : 'ok'}`}
                        onClick={() => setActiveSection('hs_vang')}
                        style={{
                            '--ms-color': totalAbsentCount > 0 ? '#be123c' : '#16a34a',
                            '--ms-bg': totalAbsentCount > 0 ? '#fff1f2' : '#f0fdf4',
                            '--ms-border': totalAbsentCount > 0 ? '#fecdd3' : '#bbf7d0'
                        }}
                        title="Xem HS Vắng"
                    >
                        <i className={`fas ${totalAbsentCount > 0 ? 'fa-user-times' : 'fa-user-check'}`}></i>
                        <span className="bctruc-mini-stat-val">{totalAbsentCount}</span>
                        <span className="bctruc-mini-stat-lbl">HS vắng</span>
                    </button>
                </div>
            </div>
            ...
            <div className="bctruc-card">
                ...
                <div className="bctruc-section-nav">
                    <div className="bctruc-section-tabs">
                        ...
                        <button
                            className={`bctruc-sec-tab tab-vang ${activeSection === 'hs_vang' ? 'active' : ''}`}
                            onClick={() => setActiveSection('hs_vang')}
                        >
                            <i className="fas fa-user-times"></i>
                            <span>HS Vắng</span>
                            <span className={`bctruc-sec-count ${totalAbsentCount > 0 ? 'danger' : ''}`}>{totalAbsentCount}</span>
                        </button>
                    </div>
                ...
                {activeSection === 'hs_vang' && (
                    <div style={{
                        ...
                        <div>
                            <div style={{ fontWeight: 800, color: '#be123c', fontSize: '0.94rem', display: 'flex', alignItems: 'center', gap: 7 }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                            <button className="bctruc-btn bctruc-btn-danger" onClick={exportLeaderReportPdf} style={{ height: 34, fontSize: '0.82rem' }}>
                                <i className="fas fa-file-invoice"></i> In báo cáo BGH
                            </button>
                        </div>
                    </div>
                )}
                                    ...
                                    <table className="bctruc-table">
                                        <thead>
                                            <tr>
                                                ...
                                                {type !== 'giamsat' && (
                                                    <th style={{ minWidth: 170, color: '#be123c' }}>
                                                        <i className="fas fa-user-times" style={{ marginRight: 4 }}></i>HS Vắng
                                                    </th>
                                                )}
                                        ...
                                        <tbody>
                                            {records.map((r, idx) => (
                                                <tr key={r.id}>
                                                    ...
                                                    {/* Cột HS Vắng (chỉ Ca Ăn và Ca Ngủ) */}
                                                    {type !== 'giamsat' && (
                                                        <td>
                                                            {r.danh_sach_vang && r.danh_sach_vang.trim() ? (
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                                        {allAbsentRecords.length === 0 ? (
                                            <div className="bctruc-empty">
                                                ...
                                                <p style={{ fontSize: '0.88rem', color: '#64748b' }}>Tất cả các phòng đều đủ sĩ số theo báo cáo của giáo viên.</p>
                                            </div>
                                        ) : (
                                            <div style={{ overflowX: 'auto', borderRadius: 8, border: '1px solid #e2e8f0' }}>
                                                <table className="table table-hover" style={{ margin: 0 }}>
                                                    <thead style={{ background: '#fff1f2' }}>
                                                        <tr>
                                                            <th style={{ width: 45, textAlign: 'center' }}>STT</th>
                                                            <th style={{ width: 85, textAlign: 'center' }}>{viewMode === 'day' ? 'Giờ gửi' : 'Thời gian'}</th>
                                                            <th style={{ width: 75, textAlign: 'center' }}>Ca</th>
                                                            <th style={{ width: 90, textAlign: 'center' }}>Phòng</th>
